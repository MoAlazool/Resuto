 ...
 with open(Path(tmp)/'server.log','w') as log:
  ...
  try:
   ...
   with sync_playwright() as p:
    ...
    guest.goto(ORIGIN+'/reserve');guest.get_by_role('button',name='Show available tables').click();guest.locator('[data-action="select-table"][data-id="t2"]').click();expect(guest.locator('#table-choice')).to_contain_text('Window side');guest.get_by_role('button',name='Review reservation').click();guest.get_by_label('Name',exact=True).fill('Reservation Guest');guest.get_by_label('Contact',exact=True).fill('fictional');guest.get_by_role('button',name='Confirm reservation',exact=True).click();guest.get_by_role('button',name='Pay test deposit').click();expect(guest.get_by_role('heading',name='Your table is confirmed')).to_be_visible();print('PASS B: next-day reservation, selection, review and test deposit',flush=True)
