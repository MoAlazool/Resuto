...
const tableKeys=['id','label','capacity','zone','shape','cx','cy','width','height','rotation','features','reservable','branchId'];
export function migrateFloor(s){
 if(s.floor?.schemaVersion===1){let changed=false;for(const t of s.tables)if(['square','round'].includes(t.shape)&&t.width!==t.height){t.height=t.width;changed=true}if(changed)s.floor.revision++;return changed;}
 for(const t of s.tables){t.cx=t.x*12;t.cy=t.y*8;t.width=110;t.height=110;t.rotation=0;}
 s.floor={schemaVersion:1,revision:0,width:1200,height:800,background:{image:s.settings.background||null,opacity:.45,locked:true,x:0,y:0,width:1200,height:800},zones:[{id:'zone-main',type:'zone',label:'Main room',x:600,y:285,width:1080,height:450,rotation:0,color:'#e9efe5'},{id:'zone-terrace',type:'zone',label:'Terrace',x:600,y:630,width:1080,height:220,rotation:0,color:'#f2eadc'}],objects:[
...
export function floorView(s){return {...structuredClone(s.floor),tables:s.tables.filter(t=>t.active).map(t=>Object.fromEntries(tableKeys.map(k=>[k,t[k]])))};}
...
export function saveFloor(s,b){
 ...
 s.floor={schemaVersion:1,revision:s.floor.revision+1,width:1200,height:800,zones,objects,background:{image:bg.image,opacity:bg.opacity,locked:bg.locked,x:bg.x,y:bg.y,width:bg.width,height:bg.height}};
 s.settings.background=bg.image;
 return floorView(s);
}
function cleanObject(o,f){if(!text(o.label,80))reject('Object label is required.');const err=geometryError(o,f);if(err)reject(o.label+': '+err);return {id:o.id,type:o.type,label:o.label,x:o.x,y:o.y,width:o.width,height:o.height,rotation:o.rotation,...(o.type==='zone'?{color:o.color}:{})};}
